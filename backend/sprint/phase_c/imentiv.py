"""Imentiv upload and polling helpers for Phase C.
...
async def get_video_emotions(settings: AISettings, video_id: str) -> list[dict[str, Any]]:
    ...
    return extract_emotions(await _poll_with_http(settings=settings, endpoint="videos", item_id=video_id))
...
async def get_audio_emotions(settings: AISettings, audio_id: str) -> list[dict[str, Any]]:
    ...
    return extract_emotions(await _poll_with_http(settings=settings, endpoint="audios", item_id=audio_id))
...
async def _poll_with_http(
    *,
    ...
    headers = {"Authorization": f"Bearer {settings.imentiv_api_key}"}
    url = f"{settings.imentiv_base_url.rstrip('/')}/{endpoint}/{item_id}/multimodal-analytics"
